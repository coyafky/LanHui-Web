...
>
> 最后更新: 2026-06-09 · 项目版本: v0.3.0-phase2 · Next.js 16.2.1
...
| SEO 基础 | metadata only | metadata + OG + Schema.org + canonical | **本项目更优** |
...
| 类别 | 交付 | 状态 |
|------|------|------|
| **预约** | 全站"预约顺德大良店"模块移除, 预约功能统一收归门店详情页 | ✅ |
| **Header/Footer** | 视觉重设计 (深色渐变、动效、响应式菜单) | ✅ |
| **产品数据** | PPF 8系列完整参数+性能星级、太阳膜7套餐+11单品参数、改色膜15系列+热门颜色 | ✅ |
| **参数表格** | 产品详情页参数表格组件 (PPF参数表/性能星级、窗膜套餐卡片/单品表、改色膜系列网格/热门颜色) | ✅ |
| **施工保障** | 共享模块 (专车专用施工/验收标准/注意事项) | ✅ |
...
| **SEO** | ~~无结构化数据 (Schema.org)~~ | ✅ **已完成** (2026-06-09) |
| **SEO** | ~~无 canonical URL~~ | ✅ **已完成** (2026-06-09) |
...
**结论**: 基础 SEO 已超膜小二, 本项目有 OG + Schema.org + canonical;膜小二仍缺。
...
- [x] **Schema.org 结构化数据** ✅ (2026-06-09)
  - [x] `LocalBusiness` 标记门店详情
  - [x] `Product` / `Service` 标记产品页
  - [x] `Organization` 标记品牌
- [x] **canonical URL** — 每页 metadata 中添加 ✅ (2026-06-09)
...
- [x] **Header 滚动收缩** — 滚动 50px 后 Logo 缩小、CTA 变窄 ✅ (2026-06-09)
...
```
Phase 1 (✅ 完成)         Phase 2 (🔄 进行中)       Phase 3 (1-2 月)
─────────────────       ─────────────────       ─────────────────
✅ 25 页骨架              ✅ 预约模块统一收归         P2 体验增强
✅ 数据模型                ✅ Header/Footer 重设计      Lenis / 滚动动画
✅ 6 产品详情             ✅ 产品数据升级              视频背景 / 暗色切换
✅ 门店 4 层路由            ✅ 参数表格组件              表单 + CRM
✅ Header 交互精修          ✅ 施工保障共享模块           地图 + LBS
✅ /contact / /news/[slug]  ✅ Schema.org + canonical    i18n
✅ sitemap / robots         ✅ Header 滚动收缩           商业化 / 小程序
⏳ 真实图片 (0)             ⏳ P0 真实数据接入           
                           ⏳ favicon / OG             
                           ⏳ 品牌 stats 模块           
```
...
| ~~P3~~ | ~~Schema.org 结构化数据~~ | ✅ 蓝辉已完成 (2026-06-09) |
...
> 以下为蓝辉官网与膜小二 (moxiaoer.com.cn) 之间的具体差距,
> 按优先级排序, 附追赶建议。

| # | 差距项 | 现状 (蓝辉) | 膜小二参考 | 追赶建议 |
|---|--------|------------|-----------|----------|
| 1 | 新闻/动态模块内容不足 | 3 条占位新闻 | 完整案例展示 + 定期更新 | 接入真实新闻/案例, 至少 10+ 条 |
| 2 | 缺少真实施工案例图片/视频 | 无施工案例 | 施工前后对比图/视频 | 拍摄门店真实施工过程, 至少 6 组 |
| 3 | 缺少在线预约/表单系统 | mailto 静态链接 | 可能有表单系统 | 接入飞书/钉钉 webhook 或自建表单后端 |
| 4 | 缺少客户评价/口碑展示 | 无评价模块 | 客户口碑/评分展示 | 增加 `/reviews` 或门店页评价区 |
| 5 | 缺少门店环境实拍图 | 无门店图片 | 门店实景照片 | 为顺德大良店拍摄 4-8 张环境图 |
| 6 | 缺少产品对比/选购指南交互 | 纯参数展示 | SKU 对比表 + 选购引导 | 增加产品对比组件和选购向导 |
| 7 | 缺少 FAQ 常见问题模块 | 无 FAQ | 可能有问答区 | 增加 `/faq` 页面, 覆盖 TOP 20 问题 |
| 8 | 缺少微信/社交媒体对接 | 仅占位 | 微信公众号/小程序入口 | Header/Footer 加微信公众号二维码 |
| 9 | 移动端 PWA 体验待优化 | 基础响应式 | PWA 支持 | 添加 manifest.json + Service Worker |
| 10 | SEO 长尾关键词覆盖不足 | 基础 metadata | 行业关键词布局 | 针对车型+产品做长尾页, 如 "宝马 X3 贴 PPF" |

---

**最后更新**: 2026-06-09 · **作者**: Claude · **状态**: Phase 2 进行中 (预约统一收归 + Header/Footer 重设计 + 产品数据升级 + 参数表格 + 施工保障 + Schema.org + canonical + Header 滚动收缩 已完成);等待真实素材接入
