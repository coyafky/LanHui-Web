...
| 状态 | 显示 |
| --- | --- |
| `matched` | 正常图片 + 车型标签 + 分类标签 + 名称 + "咨询此款" CTA |
...
本 PRD 不定义专题页私有咨询 CTA。复用全站统一的企业微信二维码、电话或联系
入口 (参照 `<PhoneCta source="wenjie_topic_*">` 组件)。
...
| `wenjie_global_consult_click` | 从问界专题触发全站统一咨询入口 |
