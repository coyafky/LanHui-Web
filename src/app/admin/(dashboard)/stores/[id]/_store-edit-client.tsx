...
export function StoreEditClient({
  ...
  id: string;
  initialStore: StoreFormValues;
}) {
  const editFormId = "store-edit-form";
  const [storeData, setStoreData] = useState<StoreFormValues>(initialStore);
  const [storeLevel, setStoreLevel] = useState<StoreLevel | null>(
    (initialStore.level ?? null) as StoreLevel | null
  );
  const [storeSlug, setStoreSlug] = useState<string | null>(
    initialStore.slug ?? null
  );
  const [storeStatus, setStoreStatus] = useState<StoreStatus | null>(
    (initialStore.status ?? null) as StoreStatus | null
  );
