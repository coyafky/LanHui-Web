...
import {
  ...
  type XiaomiSeriesCategory,
  type XiaomiSeriesScenario,
  type XiaomiSeriesUpgradeProject,
} from "@/lib/xiaomi-series-upgrade-projects";

const EXPECTED_PROJECT_COUNT = XIAOMI_SERIES_PROJECT_COUNT;

const CATEGORY_ORDER: readonly XiaomiSeriesCategory[] = [
  "protection",
  "appearance",
  "ultra_style",
  "cabin",
...
function assertProjectCount(projects: readonly XiaomiSeriesUpgradeProject[]): void {
  if (projects.length !== EXPECTED_PROJECT_COUNT) {
    throw new Error(
      `XiaomiSeriesProjectGrid expects ${EXPECTED_PROJECT_COUNT} projects, got ${projects.length}`,
    );
  }
}

export type XiaomiSeriesProjectGridProps = {
  projects: readonly XiaomiSeriesUpgradeProject[];
  scenarios: readonly XiaomiSeriesScenario[];
};

type ProjectCardProps = {
  project: XiaomiSeriesUpgradeProject;
  open: boolean;
  onToggle: () => void;
};
...
function ProjectCard({ project, open, onToggle }: ProjectCardProps) {
  const handleClick = () => {
    trackClick("xiaomi_series_project_click", {
  ...
  return (
    <article className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden flex flex-col">
        ...
        <div className="p-4">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="inline-flex items-center px-2 py-0.5 rounded-md border border-orange-900/60 text-orange-400 bg-orange-950/30 text-xs">
              {XIAOMI_SERIES_CATEGORY_LABELS[project.category]}
            </span>
      ...
      <div
        id={`xiaomi-series-project-detail-${project.id}`}
...
export function XiaomiSeriesProjectGrid({
  projects,
  scenarios,
}: XiaomiSeriesProjectGridProps) {
  assertProjectCount(projects);

  const sectionRef = useRef<HTMLElement>(null);
  const [activeCategory, setActiveCategory] = useState<XiaomiSeriesCategory | "all">("all");
  ...
  const handleHashChange = useCallback(() => {
    ...
    const projectMatch = hash.match(/^xiaomi-series-project-(.+)$/);
    if (projectMatch) {
  ...
  const scenarioFilteredProjects = useMemo<readonly XiaomiSeriesUpgradeProject[]>(() => {
  ...
  const filteredProjects = useMemo<readonly XiaomiSeriesUpgradeProject[]>(() => {
  ...
  const handleScenarioClear = useCallback(() => {
    trackClick("xiaomi_series_scenario_clear", {});
    setActiveScenarioId(null);
  ...
  const handleCategoryChange = useCallback(
    (cat: XiaomiSeriesCategory | "all") => {
      if (cat === activeCategory) return;
      trackClick("xiaomi_series_category_filter", {
        category: cat,
      });
        ...
        >
          ...
          {CATEGORY_ORDER.map((cat) => {
            ...
            return (
              ...
              >
                {XIAOMI_SERIES_CATEGORY_LABELS[cat]}（{count}）
              </button>
            );
          })}
        </div>
