...
type LiAutoSeriesMoreChoicesProps = {
  projects: readonly LiAutoSeriesUpgradeProject[];
};

const OPTIONAL_LENGTH = 30;

function assertOptionalLength(projects: readonly LiAutoSeriesUpgradeProject[]): void {
  if (projects.length !== OPTIONAL_LENGTH) {
    throw new Error(
      `LiAutoSeriesMoreChoices expects ${OPTIONAL_LENGTH} optional projects, got ${projects.length}`,
    );
  }
}
 ...
 */
export function LiAutoSeriesMoreChoices({ projects }: LiAutoSeriesMoreChoicesProps) {
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
          {projects.map((p) => (
            ...
            >
              ...
              <div className="mt-auto h-1.5 rounded-full bg-gradient-to-r from-amber-500/70 via-orange-400/30 to-transparent" />
              <p className="mt-2 text-[11px] text-zinc-500">
                可按 i8 / L9 / MEGA 车型确认适配
              </p>
            </article>
          ))}
        </div>
