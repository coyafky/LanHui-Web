...
export function DashboardRecentActivity({ data }: Props) {
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
      <h2 className="mb-4 text-lg font-semibold text-zinc-100">最近活动</h2>
      {!data || data.items.length === 0 ? (
        ...
        <ul className="divide-y divide-zinc-800">
          {data.items.map((item) => {
            ...
            const inner = (
              <>
                <span className="w-28 shrink-0 text-xs text-zinc-500">{formatRelative(item.createdAt)}</span>
                <span className="w-20 shrink-0 text-sm text-zinc-300">{item.actorName ?? "系统"}</span>
                <span
            ...
            return (
              <li key={item.id} className="flex items-center gap-3 py-2.5 first:pt-0 last:pb-0">
                {href ? <Link href={href} className="flex w-full items-center gap-3 hover:opacity-80">{inner}</Link> : <div className="flex w-full items-center gap-3">{inner}</div>}
              </li>
            );
          })}
        </ul>
