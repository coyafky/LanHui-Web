...
type XpengGxScenarioMatrixProps = {
...
function assertScenarioLength(scenarios: readonly XpengGxScenario[]): void {
  if (scenarios.length !== SCENARIO_LENGTH) {
    throw new Error(
      `XpengGxScenarioMatrix expects ${SCENARIO_LENGTH} scenarios, got ${scenarios.length}`,
    );
  }
}
 ...
 * SPEC §3.2：3 列 / md:3 / sm:2 / mobile:1，每卡：场景名 + 描述 + 项目数 + 前 4 项目名 + "查看完整方案"
 * 点击 → 滚动到 xpeng-gx-scenario-group-{key} + 触发埋点
 */
export function XpengGxScenarioMatrix({
  ...
  function handleScenarioClick(scenario: XpengGxScenario) {
    const targetId = `xpeng-gx-scenario-group-${scenario.key}`;
  ...
  return (
    <section
      ...
      aria-labelledby="xpeng-gx-scenarios-heading"
    >
          ...
          {scenarios.map((s) => {
            ...
            return (
              <button
                ...
                className="group bg-zinc-900 rounded-2xl border border-zinc-800 hover:border-emerald-700/60 transition-colors p-5 flex flex-col text-left"
              >
                ...
                <span className="inline-flex self-start items-center px-2 py-0.5 rounded-md text-xs font-medium bg-emerald-950/40 border border-emerald-900/60 text-emerald-400 mb-3">
